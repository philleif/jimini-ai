...
indicators.strategy = async candles => {
  try {
    // for each candle
      // if the next candle's price is higher, set strategy to LONG/1
      // else, set it to SHORT/0
    for (let i = 0; i < candles.length - 1; i++) {
        let currentPrice = candles[i].close
        let nextPrice = candles[i+1].close

        if (currentPrice <= nextPrice) {
          candles[i].strategy = "LONG"
          candles[i].strategyCode = 1
        } else {
          candles[i].strategy = "SHORT"
          candles[i].strategyCode = 0
        }
    }

    // we won't have values for the last row
    candles[candles.length - 1].strategy = ""
    candles[candles.length - 1].strategyCode = ""
