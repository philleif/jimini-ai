...
indicators.upDownThreeMethods = async candles => {
  try {
    let indicator = await getTalibIndicator("upDownThreeMethods", "CDLXSIDEGAP3METHODS", candles)
    candles = await addIndicatorToCandles("upDownThreeMethods", indicator, candles)
...
indicators.upsideGapTwoCrows = async candles => {
  try {
    let indicator = await getTalibIndicator("upsideGapTwoCrows", "CDLUPSIDEGAP2CROWS", candles)
    candles = await addIndicatorToCandles("upsideGapTwoCrows", indicator, candles)
...
indicators.uniqueThreeRiver = async candles => {
  try {
    let indicator = await getTalibIndicator("uniqueThreeRiver", "CDLUNIQUE3RIVER", candles)
    candles = await addIndicatorToCandles("uniqueThreeRiver", indicator, candles)
...
indicators.thrusting = async candles => {
  try {
    let indicator = await getTalibIndicator("thrusting", "CDLTHRUSTING", candles)
...
indicators.stickSandwich = async candles => {
  try {
    let indicator = await getTalibIndicator("stickSandwich", "CDLSTICKSANDWICH", candles)
...
indicators.stalledPattern = async candles => {
  try {
    let indicator = await getTalibIndicator("stalledPattern", "CDLSTALLEDPATTERN", candles)
...
indicators.spinningTop = async candles => {
  try {
    let indicator = await getTalibIndicator("spinningTop", "CDLSPINNINGTOP", candles)
...
indicators.shortLineCandle = async candles => {
  try {
    let indicator = await getTalibIndicator("shortLineCandle", "CDLSHORTLINE", candles)
...
indicators.shootingStar = async candles => {
  try {
    let indicator = await getTalibIndicator("shootingStar", "CDLSHOOTINGSTAR", candles)
...
indicators.separatingLines = async candles => {
  try {
    let indicator = await getTalibIndicator("separatingLines", "CDLSEPARATINGLINES", candles)
...
indicators.riseFallThreeMethods = async candles => {
  try {
    let indicator = await getTalibIndicator("riseFallThreeMethods", "CDLRISEFALL3METHODS", candles)
    candles = await addIndicatorToCandles("riseFallThreeMethods", indicator, candles)
...
indicators.rickshawMan = async candles => {
  try {
    let indicator = await getTalibIndicator("rickshawMan", "CDLRICKSHAWMAN", candles)
...
indicators.morningStar = async candles => {
  try {
    let indicator = await getTalibIndicator("morningStar", "CDLMORNINGSTAR", candles)
...
indicators.morningDojiStar = async candles => {
  try {
    let indicator = await getTalibIndicator("morningDojiStar", "CDLMORNINGDOJISTAR", candles)
...
indicators.matchingLow = async candles => {
  try {
    let indicator = await getTalibIndicator("matchingLow", "CDLMATCHINGLOW", candles)
...
indicators.longLineCandle = async candles => {
  try {
    let indicator = await getTalibIndicator("longLineCandle", "CDLLONGLINE", candles)
...
indicators.longLeggedDoji = async candles => {
  try {
    let indicator = await getTalibIndicator("longLeggedDoji", "CDLLONGLEGGEDDOJI", candles)
...
indicators.ladderBottom = async candles => {
  try {
    let indicator = await getTalibIndicator("ladderBottom", "CDLLADDERBOTTOM", candles)
...
indicators.kickingByLength = async candles => {
  try {
    let indicator = await getTalibIndicator("kickingByLength", "CDLKICKINGBYLENGTH", candles)
...
indicators.invertedHammer = async candles => {
  try {
    let indicator = await getTalibIndicator("invertedHammer", "CDLINVERTEDHAMMER", candles)
...
indicators.identicalThreeCrows = async candles => {
  try {
    let indicator = await getTalibIndicator("identicalThreeCrows", "CDLIDENTICAL3CROWS", candles)
    candles = await addIndicatorToCandles("identicalThreeCrows", indicator, candles)
...
indicators.homingPigeon = async candles => {
  try {
    let indicator = await getTalibIndicator("homingPigeon", "CDLHOMINGPIGEON", candles)
...
indicators.hikkakeModified = async candles => {
  try {
    let indicator = await getTalibIndicator("hikkakeModified", "CDLHIKKAKEMOD", candles)
...
indicators.haramiCross = async candles => {
  try {
    let indicator = await getTalibIndicator("haramiCross", "CDLHARAMICROSS", candles)
...
indicators.hangingMan = async candles => {
  try {
    let indicator = await getTalibIndicator("hangingMan", "CDLHANGINGMAN", candles)
...
indicators.gravestoneDoji = async candles => {
  try {
    let indicator = await getTalibIndicator("gravestoneDoji", "CDLGRAVESTONEDOJI", candles)
...
indicators.gapSideSideWhite = async candles => {
  try {
    let indicator = await getTalibIndicator("gapSideSideWhite", "CDLGAPSIDESIDEWHITE", candles)
    candles = await addIndicatorToCandles("gapSideSideWhite", indicator, candles)
...
indicators.eveningStar = async candles => {
  try {
    let indicator = await getTalibIndicator("eveningStar", "CDLEVENINGSTAR", candles)
...
indicators.eveningDojiStar = async candles => {
  try {
    let indicator = await getTalibIndicator("eveningDojiStar", "CDLEVENINGDOJISTAR", candles)
...
indicators.engulfingPattern = async candles => {
  try {
    let indicator = await getTalibIndicator("engulfingPattern", "CDLENGULFING", candles)
    candles = await addIndicatorToCandles("engulfingPattern", indicator, candles)
...
indicators.dragonflyDoji = async candles => {
  try {
    let indicator = await getTalibIndicator("dragonflyDoji", "CDLDRAGONFLYDOJI", candles)
...
indicators.counterattack = async candles => {
  try {
    let indicator = await getTalibIndicator("counterattack", "CDLCOUNTERATTACK", candles)
...
indicators.concealingBabySwallow = async candles => {
  try {
    let indicator = await getTalibIndicator("concealingBabySwallow", "CDLCONCEALBABYSWALL", candles)
    candles = await addIndicatorToCandles("concealingBabySwallow", indicator, candles)
...
indicators.closingMarubozu = async candles => {
  try {
    let indicator = await getTalibIndicator("closingMarubozu", "CDLCLOSINGMARUBOZU", candles)
...
indicators.breakaway = async candles => {
  try {
    let indicator = await getTalibIndicator("breakaway", "CDLBREAKAWAY", candles)
...
indicators.advanceBlock = async candles => {
  try {
    let indicator = await getTalibIndicator("advanceBlock", "CDLADVANCEBLOCK", candles)
...
indicators.abandonedBaby = async candles => {
  try {
    let indicator = await getTalibIndicator("abandonedBaby", "CDLABANDONEDBABY", candles)
...
indicators.threeWhiteSoldiers = async candles => {
  try {
    let indicator = await getTalibIndicator("threeWhiteSoldiers", "CDL3WHITESOLDIERS", candles)
    candles = await addIndicatorToCandles("threeWhiteSoldiers", indicator, candles)
...
indicators.threeStarsInTheSouth = async candles => {
  try {
    let indicator = await getTalibIndicator("threeStarsInTheSouth", "CDL3STARSINSOUTH", candles)
    candles = await addIndicatorToCandles("threeStarsInTheSouth", indicator, candles)
...
indicators.threeOutsideUpDown = async candles => {
  try {
    let indicator = await getTalibIndicator("threeOutsideUpDown", "CDL3OUTSIDE", candles)
    candles = await addIndicatorToCandles("threeOutsideUpDown", indicator, candles)
...
indicators.threeLineStrike = async candles => {
  try {
    let indicator = await getTalibIndicator("threeLineStrike", "CDL3LINESTRIKE", candles)
...
indicators.threeInsideUpDown = async candles => {
  try {
    let indicator = await getTalibIndicator("threeInsideUpDown", "CDL3INSIDE", candles)
    candles = await addIndicatorToCandles("threeInsideUpDown", indicator, candles)
...
indicators.darkCloudCover = async candles => {
  try {
    let indicator = await getTalibIndicator("darkCloudCover", "CDLDARKCLOUDCOVER", candles)
